...
// Prefer the dedicated smoke canvas if present, else fall back to first canvas
var canvas = document.getElementById('smoke-canvas') || document.getElementsByTagName('canvas')[0];
...
function getWebGLContext( canvas ) {
    ...
    // White background for the final render
...
// Display shader: composite blue-tinted smoke over white background
var displayShader                  = compileShader( gl.FRAGMENT_SHADER,
    ...
    '    float t = clamp(length(d) * 1.0, 0.0, 1.0);\n' +
    '    vec3 c1 = vec3(0.92, 0.97, 1.0); /* very light baby blue */\n' +
    '    vec3 c2 = vec3(0.65, 0.82, 1.0); /* light/medium blue */\n' +
    '    vec3 smoke = mix(c1, c2, t);\n' +
    '    vec3 bg = vec3(1.0); /* white background */\n' +
    '    vec3 color = mix(bg, smoke, t * 0.6); /* softer overall intensity */\n' +
...
function splat( x, y, dx, dy, color ) {
    ...
    // Inject grayscale density so display shader controls the blue tint
    var densityStrength = (color[0] + color[1] + color[2]) / 3 * 0.25;
    gl.uniform3f( splatProgram.uniforms.color, densityStrength, densityStrength, densityStrength );
    blit( density.second[ 1 ] );
    density.swap();
...
// Helper: is pointer currently over a product card?
function isOverProductCardAt(x, y, fallbackTarget) {
    var el = (typeof x === 'number' && typeof y === 'number')
        ? document.elementFromPoint(x, y)
        : fallbackTarget;
    if (!el) return false;
    return !!(el.closest && el.closest('.product-card'));
}

// Listen on document to capture interactions even if canvas ignores events
document.addEventListener( 'mousemove', function ( e ) {
    // Don’t track when hovering over product cards
    if (isOverProductCardAt(e.clientX, e.clientY, e.target)) return;
...
document.addEventListener( 'touchmove', function ( e ) {
    ...
    for ( var i = 0, len = touches.length; i < len; i++ ) {
        ...
        var t = touches[i];
        // Skip tracking for touches over product cards
        if (isOverProductCardAt(t.clientX, t.clientY, t.target)) {
            continue;
        }

        pointers[ i ].id    = t.identifier;
        pointers[ i ].down  = true;
        pointers[ i ].x     = t.pageX;
        pointers[ i ].y     = t.pageY;
        pointers[ i ].color = colorArr;

        var pointer = pointers[ i ];

    pointer.moved = pointer.down;
    pointer.dx    = (t.pageX - pointer.x) * 10.0;
    pointer.dy    = (t.pageY - pointer.y) * 10.0;
    pointer.x     = t.pageX;
    pointer.y     = t.pageY;
...
window.onload = function() {
    // Only run letter transition if an element with id="h1" exists
    var titleEl = document.getElementById('h1');
    if (titleEl) {
        m("h1");
    }
    // Seed some random splats so smoke is visible on load
    splatStack.push(30);
};